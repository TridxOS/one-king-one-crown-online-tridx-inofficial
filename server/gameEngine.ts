...
function completeTurn(state: GameState, actor: PlayerState) {
  ...
  state.currentPlayerId = next.id; state.playsRemaining = 2;
...
export function adjustGold(state: GameState, actorId: string, targetId: string, amount: number, reason: string) { if (state.status !== "playing") throw new Error("Start the game before adjusting gold."); player(state, actorId); const target = player(state, targetId); const normalized = Math.max(-3000, Math.min(3000, Math.round(amount / 100) * 100)); if (!normalized) throw new Error("Enter a gold amount in steps of 100."); target.gold = clampGold(target.gold + normalized); resolveCrownChange(state); event(state, `${target.name} ${normalized > 0 ? "gained" : "lost"} ${Math.abs(normalized)} gold${reason ? ` — ${reason.slice(0, 80)}` : ""}.`, "gold"); }
export function transferGold(state: GameState, actorId: string, fromId: string, toId: string, amount: number, reason: string) { if (state.status !== "playing") throw new Error("Start the game before transferring gold."); player(state, actorId); const from = player(state, fromId); const to = player(state, toId); if (from.id === to.id) throw new Error("Choose two different nobles."); const proposed = Math.max(100, Math.min(3000, Math.round(amount / 100) * 100)); const paid = Math.min(from.gold, proposed); from.gold -= paid; to.gold += paid; resolveCrownChange(state); event(state, `${from.name} paid ${to.name} ${paid} gold${reason ? ` — ${reason.slice(0, 80)}` : ""}.`, "gold"); }
